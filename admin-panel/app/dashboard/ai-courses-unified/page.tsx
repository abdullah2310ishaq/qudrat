...
export default function UnifiedAICoursesPage() {
  const toast = useToast();
  const [courses, setCourses] = useState<AICourse[]>([]);
  const [filteredCourses, setFilteredCourses] = useState<AICourse[]>([]);
  ...
  useEffect(() => {
    if (searchQuery.trim() === '') {
      ...
      const filtered = courses.filter(course => 
        ...
        course.heading.toLowerCase().includes(query) ||
        course.aiTool.toLowerCase().includes(query) ||
        (course.subHeading && course.subHeading.toLowerCase().includes(query)) ||
  ...
  return (
    <div className="p-4 min-h-screen" style={{ backgroundColor: '#F5F5DC' }}>
      {/* Header */}
      <div className="mb-4">
        <h1 className="text-xl font-bold text-black mb-1">🤖 AI Mastery Paths</h1>
        <p className="text-black/70 text-xs">Manage AI courses with levels and lessons</p>
      </div>

      {/* Action Buttons */}
      <div className="flex gap-2 mb-4">
        <Link
          href="/dashboard/ai-courses-unified/add"
          className="px-3 py-1.5 bg-black text-white rounded hover:bg-black/90 transition-all text-xs font-semibold"
        >
          ➕ Add New AI Course
        </Link>
      </div>
      ...
      {/* Courses Grid */}
      {searchQuery && filteredCourses.length === 0 && courses.length > 0 ? (
        <div className="text-center py-8 bg-white rounded border border-black/20">
          <div className="text-3xl mb-2">🔍</div>
          <p className="text-black text-sm font-bold mb-1">No courses found</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
          ...
          <div className="col-span-full text-center py-8 bg-white rounded border border-black/20">
            ...
            <p className="text-black text-base font-bold mb-2">No AI courses yet</p>
            <p className="text-black/60 text-xs mb-3">Create your first AI mastery path to get started</p>
            <Link
              href="/dashboard/ai-courses-unified/add"
          ...
          courses.map((course) => {
            const totalLessons = course.tree?.reduce((sum, level) => sum + (level.lessons?.length || 0), 0) || 0;
            return (
              <div
                key={course._id}
              ...
              >
                {/* Course Cover Photo */}
                <Link
                  href={`/dashboard/ai-courses-unified/view/${course._id}`}
                  className="block"
                >
                ...
                <div className="p-3">
                  <Link href={`/dashboard/ai-courses-unified/view/${course._id}`}>
                  ...
                  <div className="flex gap-1.5 pt-2 border-t border-black/10">
                    ...
                    <button
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        window.location.href = `/dashboard/ai-courses-unified/edit/${course._id}`;
                      }}
